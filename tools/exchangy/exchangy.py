...
# define option args
  ...
  optupdt = 0
  #try:
  opts,args = getopt.getopt(sys.argv[1:],'t:p:hsu')
  #except:
  ...
  for opt, arg in opts:
    ...
    if opt in ('-u'):
      optupdt = 1
  return opthalp, opttrgt, optport, optskip, optupdt


def update():
  global db, upd_packet
  err = ''
  ...
  update = ''
  i = 0
  while i < 3301: # emerge...ncy exit incase we dont receive expected end of packet
    try:
...
# parse option args and load prepare init
  ...
  if (optupdt == 0): # 0 for flase, user did not choose to skip update
  ...
  try:
    ...
    print('   https://github.com/Haxel0rd/haxel0rds/upload/master/tools/exchangy/')
  ...
  if (optskip == 1): # 1 for true, user skipped update
    skipOnline = True
  ison()
  # Drop initialize msg
  ...
  if (skipOnline == False):
    ...
    if (isOnline == False):
      try:
        ...
        pass
    if (isOnline == True):
      print('** Online probe: target is online')
    else:
...
# lookup msdocs for owa build numbers
  ...
  if (xchConfirmed == False):
    tmp = owa.rsplit('.',1)[0] # No luck in first try, take away the last number of the owa build and see if we match then...
    for item in msdb:
      try:
