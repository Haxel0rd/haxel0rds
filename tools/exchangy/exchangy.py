...
#   > (i see this faster than github). Thank you.                            #
#                                                                            #
# ------------------------------------------------------------               #
# Exchangy - Exchange Server version & patchlevel detection                  #
# ------------------------------------------------------------               #
# - CreationDate: 06/2020, version: v1.00                                    #
# - Recoded in 01/2021, version: v2.0                                        #
#   (i felt this new version is the true v1.00, before was beta ;)           #
# - Cross Compatibilty: Win, Linux / py3 (tested: 2.7.13 & 3.7.0)      #
...
# - Incase OWA not found, a fallback method via POP3 is fired                #
# - Supported Patchlevels are:                                               #
#   ServicePack (SP), CumulativeUpdates (CU) and RollupUpdates (RU).         #
# - Can deal with even with most custom and "cleansed" owa panels            # 
# - Bypasses some things, except F5 Bot-Protection                           #
...
onc_packet = 'GET /index.php HTTP/1.1\r\nHost: {{target}}\r\nUser-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:72.0) Gecko/20100101 Firefox/72.0\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8\r\nAccept-Language: en-US,en;q=0.5\r\nAccept-Encoding: identity\r\nUpgrade-Insecure-Requests: 1\r\nConnection: close\r\nDNT: 1\r\n\r\n'
...
# parse option args and load prepare init
  global owa, target, port, enterMode, skipOnline
  target = ''
  rport = ''
  options = args()
  opthalp = options[0]
  ...
  # Check if help was called
    ...
    print('   -e    optional: enter OWA build number manually')
    print('   -------------------------------------------------')
    print('** Detections are runtime updated, but not tool. For tool go here:')
  ...
  try:
    ...
    print('   https://github.com/Haxel0rd/haxel0rds/tree/master/tools/exchangy')
    print('   If this error persists for over a few days, please contact the tools')
    print('   author at twitter: https://www.twitter.com/haxel0rd - Thank you.\n')
    print('** May the force be with you.')
    sys.exit('\n\n')
  ...
  for item in msdb:
    icounter+=1
  print('** Currently covering '+str(icounter)+' items,') 
  print('   Latest Patchlevel: '+msdb[item]["name"].replace('Exchange Server','Exchange')+', '+msdb[item]["release"]) 
  ...
  try:
    ...
    newVersion = newVersion["toolinfo"]["tool_version"]
    if (newVersion != msdb["toolinfo"]["tool_version"]):
  ...
  if (optentr == 1):
    ...
    while True:
      ...
      if not usr:
        usr = 'a'
      if (re.search('[^0-9.]',usr) == None and len(str(usr)) < 18):
        owa = usr
        mslookup()
        rgen()
        sys.exit()
      else:
        print('** Error: only numbers and dots are allowed,')
        print('   max chars is 17, try again cowboy ...')
  ...
  if (optport == ''):
    ...
    if (optport != 0):
      ...
      if (port > 65535 or port < 1):
        ...
        sys.exit('** Cannot continue, exiting.\n')
      prt = '   Port   set to: '+str(optport)+' (user defined)'
  host = (target,port)
  # Drop initialize msg
...
# check if target is online
  ...
  if (skipOnline == False):
    ...
    if (isOnline == True):
      ...
      print('** Target looks offline from here, if you believe this is an')
      print('   error, re-run the tool with -s to skip targets online check')
      sys.exit('** Cannot continue, exiting.\n')


def checks():
# check owa method 1, logon.aspx
  global owa, owaConfirmed
  owaConfirmed = False
  packet = owa_packet.replace('{{target}}',target)
  owa_response = netw(host,packet)
  if (owa_response == ''):
    ...
    if (owa_regex2 and owaConfirmed == False):
      print('** Couldn\'t yet detect OWA panel, sending more checks')
    owa_regex = re.search('\"/owa/auth/([0-9]{1,}\.[0-9]{1}\.[0-9]{3,}(.{1,}|))/themes/',str(owa_response))
    if owa_regex:
      owa = owa_regex.group(1).split('/')[0]
      owaConfirmed = True
    else:
      if (owaConfirmed == True):
        print('** Failed fetching OWA version, panel may have')
        print('   been obfuscated by admins.. trying to bypass')
      else:
...
# nothing found in first run, fire a second round with different approach
  ...
  owa_response2 = netw(host,packet)
  protected = re.search('Please enable JavaScript to view the page content',str(owa_response2))
  if protected:
    print('** ALERT: Targetsite is using WAF protection! A bypass may be')
    print('   added in future versions, but this case occurs rarely atm.')
  ...
  owa_regex2 = re.search('\"/owa/auth/([0-9]{1,}\.[0-9]{1}\.[0-9]{3,}(.{1,}|))/themes/',str(owa_response2))
  if not owa_regex2:
    # 3rd method, raising server exception, re-using url from method 2, 
    # calling non existant .aspx file (we barely need this), will result in owa buildnumber leak:
    owa_regex3 = re.search('\"/owa/([0-9]{1,}\.[0-9]{1}\.[0-9]{3,}(.{1,}|))/themes/',str(owa_response2))
    if not owa_regex3:
      print('** Unable to detect OWA at target server :(')
      ...
      owa = str(owa_regex3.group(1)).split('/themes')[0]
      owaConfirmed = True
  else:
    print('** OWA version found, trying Exchange version next')
    owa = str(owa_regex2.group(1)).split('/themes')[0]
    owaConfirmed = True
...
# lookup msdocs for owa build numbers
  ...
  if (owa != ''):
    for item in msdb:
      # 4 debugging regex search
      # x = re.search(rf"{owa}",db[item]["buildno_s"][:-3])
      # print(str(item)+' Owa:'+owa+' cu:'+db[item]["release"]+' db:'+db[item]["buildno_s"][:-2]+' rgx: '+str(x))
      try:
    ...
    if (xchConfirmed == False):
      tmp = owa.replace('0','') # No luck in first try, fire a second round with minified version number
      for item in msdb:
        try:
          if re.search(tmp,msdb[item]["buildno_s"]):
            ...
            pass
    if (xchConfirmed == False):    
      tmp = tmp.rsplit('.',1)[0] # last try, taking away the last number of the buildid and see if we match then... 
      for item in msdb:
        try:
          if re.search(tmp,msdb[item]["buildno_s"]):
            xch = msdb[item]["name"]
...
# WE ARE DONE - PRINTING OUT RESULTS AND GTFO HERE LOL
  ...
  if (xch == '<unknown>' and owa != '<unknown>'):
      ...
      print('** Use the OWA buildnumber for manual research.')
