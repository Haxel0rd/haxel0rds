...
#   > (i see this faster than on github). Thank you.                         #
#                                                                            #
# ------------------------------------------------------------               #
# Exchangy - Exchange Server version & patchlevel detection                  #
# ------------------------------------------------------------               #
# - Creation: 06/2020, version: v0.09beta (accidentally released as 1.00)    #
# - Recoded in 01/2021, version: v1.0 ..(the real version 1.00)              #
...
#   build number with MS-docs. Patchlevel does not detect KB patches!        #
...
# - Bypasses some things, except F5 Bot-Protection adn layer-3 firewalls     #
...
try:
  ...
  sys.exit('   Get this at https://pypi.org/ or install a fresh python 3.7')
try:
  import ssl
except:
...
owaConfirmed = False
xchConfirmed = False
ExchangyVersion = 'v1.00' # REMINDER: 
...
# parse option args and load prepare init
  ...
  print('   Latest Patchlevel: '+msdb[item]["name"]+', '+msdb[item]["release"]) 
  ...
  if (target == ''):
    ...
    print('   Must be URL or IP adress (custom ports are supported)')
    print('   Sorry we can\'t continue, exiting ...')
    sys.exit('** May the force be with you.\n\n')
...
# check if target is online
  ...
  if (skipOnline == False):
    ...
    if (isOnline == True):
      ...
      print('** Target looks unavailable at given address, if you believe this is')
      print('   an error, re-run the tool with -s to skip targets online check')
      print('   Sorry we can\'t continue, exiting ...')
      sys.exit('** May the force be with you.\n\n')
  ...
  if not owa_regex_gbn1:
    ...
    if not owa_regex_gbn2:
      ...
      if (owaConfirmed == False):
        print('   (maybe not an Exchange Server)')
      else:
        print('   Try adding build number manually with -e ')
      print('   Sorry we can\'t continue, exiting ...')
      sys.exit('** May the force be with you.\n\n')
